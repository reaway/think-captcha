...
namespace Think\Component\Captcha;
...
class Captcha
{
     ...
     */
    protected function configure(string|null $config = null): void
    {
        if (is_null($config)) {
     ...
     * @return Response
     */
    public function create(string|null $config = null, bool $api = false): Response
    {
        ...
        // 验证码使用随机字体
        $ttfPath = dirname(__DIR__) . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . ($this->useZh ? 'zhttfs' : 'ttfs') . DIRECTORY_SEPARATOR;
    ...
    {
        $path = dirname(__DIR__) . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'bgs' . DIRECTORY_SEPARATOR;
        $dir = dir($path);
